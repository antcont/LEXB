...
def scraper(URL_list, export_folder):
    ...
    for id, url in enumerate(URL_list):                     # iterating over URL list; 'id' will be part of the filename
        ...
        if title_ is not None:
            title = title_.get_text(strip=True)                               # getting the title and stripping whitespaces
            text_as_list.append(title)                                        # adding the title
        ...
        if not text_as_string:                                                # raising error message if text is empty
            print("ERROR: STPLC_IT_%(text_id)s.txt not downloaded. URL: %(url)s " % {"text_id": str(id).zfill(4), "url": url})
            df_.loc[len(df_.index)] = ["ERROR! No text downloaded: STPLC_IT_%s.txt" % str(id).zfill(4), url, title,
                                       "-", "ERROR: empty"]                   # adding error info on report file
        else:
            detected_lang = detect(text_as_string)                            # detecting text language
            newfile_path = export_folder + "\STPLC_IT_%s.txt" % str(id).zfill(4)  # building filename as "STPLC_IT_0000"
            with open(newfile_path, "w", encoding="utf-8", newline="\n") as file:
                file.write(text_as_string)                                    # writing the .txt file
            print("[%(id)i/%(tot)i] %(url)s downloaded to %(filename)s" % {"id": id+1, "tot": len(URL_list), "url": url, "filename": newfile_path})
            df_.loc[len(df_.index)] = ["STPLC_IT_%s.txt" % str(id).zfill(4), url, title, detected_lang, "OK"] # info on report file
