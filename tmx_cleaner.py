...
tmx_path = r"C:\Users\anton\Desktop\prove_download_scraper\prova19.10\merged_tmx19 - Copia_prove_cleaning.19.11.tmx"                                  # insert path of the .tmx file
...
def remove_art_and_co(path):
    counter_art_mod = 0
    counter_art_rem = 0
    regex_mod = regex.compile(r"^(Art\. \d{1,2}) ?(\- )?(\(?\p{Lu}.+)")       #segments beginning with "Art. 1" or "Art. 1 - "
...
patterns = [
    regex.compile(r"^((.+))$"),                       # removing strange "" sign at the end of some segments
    regex.compile(r"^(\(\d{1,2}\)) ?(\p{Lu}.+)"),       # removing "(1) " from beginning of segment
    regex.compile(r"^[“„'\"]?(\d{1,2}|[a-z]{1,2})\.?\)? ?(\p{Lu}\p{Ll}.+)[“„'\"]?"),      #removing "1.", "a.", "1)", "a)" (even when preceded by apostrophes or quotation marks)  # added final square parentheses to eliminate eventual final quotation marks, (TO TEST!, o perlomeno da rifare tutti su dataset sporco)
    ...
    regex.compile(r"^(\()(.+)\)$"),                     # removeing parentheses when both at beginning and end of segment
    regex.compile(r"^[“„'\"]?\d{1,2}/?(bis|ter|quater|quinquies|sexies|septies|octies|novies|decies|undecies|duodecies) (/p{Lu}.+)"), #removing instances like "5/bis " at the beginning of the segment
    regex.compile(r"^(.''\) )(\p{Lu}.+)")               # removing other noise (two apostrophes and a closed parenthesis) at the beginning of titles
]
...
def noise_cleaning(path, regexes):
    ...
    tree = etree.parse(path)
    root = tree.getroot()
    print("Another segment cleaning stage...")
    for tu in root.iter("tu"):
        for tuv in tu.iter("tuv"):
            seg = tuv.find("seg")
            seg_t = seg.text
...
'''Removing 1:0 TUs'''          # LF aligner already eliminates them.
...
def segment_counter(path):
    ...
    for tu in root.iter("tu"):
        ...
        if 20 <= len(source_tokens) <= 1000 and 20 <= len(target_tokens) <= 1000:
